...
- **0.4** Clone and verify single-image inference for all external detectors (expanded to 6):
...
- **1.1** Create `scripts/characterize_datasets.py`. For each dataset × split, compute:
...
- **1.2** Plot mean radial spectrum: real vs fake per dataset. HF L2 divergence (>80% Nyquist): CIFAKE=0.34, Defactify=4.23, GenImage=6.16
- **1.3** GenImage run locally on Modal (data directly accessible — no separate Modal script needed)
- **1.4** Produce **Figure 1**: `figures/fig1_radial_spectra.png` — 3-panel mean radial spectra with ±1σ shading. Also `figures/fig1b_2d_heatmaps.png` (supplementary). Spectra saved to `results/spectra/*.npz`.
